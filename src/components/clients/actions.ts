...
export const fetchClients = createServerFn().handler(async () => {
  ...
  const allCounterpartyIds = [
    ...new Set(
      rows.flatMap((row) =>
        row.counterparties.map((item) => item.counterparty.id),
      ),
    ),
  ]
  ...
  for (const row of blockedContractRows) {
    const existing =
      blockedContractIdsByCounterpartyId.get(row.counterpartyId) ??
      new Set<string>()
  ...
  return rows.map((row) => {
    ...
    for (const item of row.counterparties) {
      const blockedForCounterparty = blockedContractIdsByCounterpartyId.get(
...
export const fetchClientsWithSummary = createServerFn().handler(
  async (): Promise<ClientSummary[]> => {
    const session = await requireSession()
    const request = await getRequest()
    ...
    const allCounterpartyIds = [
      ...new Set(
        clientRows.flatMap((row) =>
          row.counterparties.map((c) => c.counterparty.id),
        ),
      ),
    ]
      ...
      allContractIds.length === 0
        ...
        : await db
            ...
            .innerJoin(
              ...
              and(
                eq(
                  contractPriceRevisionItem.revisionId,
                  contractPriceRevision.id,
                ),
                isNull(contractPriceRevision.completedAt),
              ),
            )
            .where(
              inArray(contractPriceRevisionItem.contractId, allContractIds),
            )
    ...
    return clientRows.map((row) => {
      ...
      const totalRevenue = invoiceRows
        .filter(
          (inv) =>
            inv.counterpartyId && counterpartyIdSet.has(inv.counterpartyId),
        )
        .reduce((sum, inv) => {
      ...
      const blockedServicesCount = [...new Set(clientContractIds)].filter(
        (id) => blockedContractIdSet.has(id),
      ).length
...
export const updateClient = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    await db.transaction(async (tx) => {
      ...
      await tx.delete(clientManager).where(eq(clientManager.clientId, data.id))
    ...
    const revisionRows =
      contractIds.length === 0
        ...
        : await db
            ...
            .innerJoin(
              contract,
              eq(contractPriceRevisionItem.contractId, contract.id),
            )
