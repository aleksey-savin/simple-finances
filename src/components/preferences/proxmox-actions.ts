...
export const fetchProxmoxNodes = createServerFn().handler(async () => {
  ...
  return db.select().from(proxmoxNode).where(eq(proxmoxNode.currentAccountId, accountId))
})
...
export const addProxmoxNode = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    const accountId = await getAccountId()
    await db.insert(proxmoxNode).values({ ...data, currentAccountId: accountId })
  })

export const updateProxmoxNodeSchema = proxmoxNodeSchema.extend({ id: z.string() })
...
export const updateProxmoxNode = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    await db
      .update(proxmoxNode)
      .set(fields)
      .where(and(eq(proxmoxNode.id, id), eq(proxmoxNode.currentAccountId, accountId)))
  })
...
export const deleteProxmoxNode = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    await db
      .delete(proxmoxNode)
      .where(and(eq(proxmoxNode.id, data.id), eq(proxmoxNode.currentAccountId, accountId)))
  })
...
export const fetchProxmoxAccountSettings = createServerFn().handler(async () => {
  const accountId = await getAccountId()
  ...
  .handler(async ({ data }) => {
    ...
    if (existing.length > 0) {
      await db
        ...
        .set({ reminderDaysBefore: data.reminderDaysBefore, updatedAt: new Date() })
        .where(eq(proxmoxAccountSettings.currentAccountId, accountId))
    } else {
