...
import { db } from '#/db'
import { expense, currentAccountUser, currentAccount } from '#/db/schema'
...
type ExpenseRow = Awaited<ReturnType<typeof fetchPayables>>['rows'][number]
...
const overdueFilterFn: FilterFn<ExpenseRow> = (
  row,
  _columnId,
  value: boolean,
) => {
  ...
  const columns = useMemo<ColumnDef<ExpenseRow, unknown>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ getValue }) => (
          <span className="font-semibold tabular-nums text-red-500 block text-right">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-semibold">Кредиторская задолженность</h1>
          </div>
          <p className="text-sm text-muted-foreground mt-1">
            Неоплаченные расходы — должны мы
          </p>
        </div>
        <div className="flex gap-3">
          <div className="rounded-lg border px-4 py-2 text-sm">
            <p className="text-muted-foreground">Всего к оплате</p>
...
function Toolbar({
  ...
  const hasFilters = globalFilter || accountFilter || categoryFilter || overdueOnly
  ...
  const filteredOverdue = filteredRows.filter((r) =>
